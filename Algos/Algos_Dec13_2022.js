...
class MinHeap {
    ...
    extract() {
        ...
        while (idxOfNodeToShiftDown < this.heap.length-1) {
            const indexOfLeftChild = this.idxOfLeftChild(idxOfNodeToShiftDown);
            const indexOfRightChild = this.idxOfRightChild(idxOfNodeToShiftDown);
            const indexToSwap = this.heap[indexOfLeftChild] < this.heap[indexOfRightChild]? indexOfLeftChild : indexOfRightChild;
            if(this.heap[idxOfNodeToShiftDown]> this.heap[indexToSwap]){
                this.swap(indexToSwap,idxOfNodeToShiftDown);
                idxOfNodeToShiftDown= indexToSwap;
            }
            else{
                break;
            }
    ...
    const MyNewHeap = new MinHeap();
    MyNewHeap.insert(2).insert(1).insert(3).insert(4);
    MyNewHeap.printHorizontalTree();
    console.log(MyNewHeap.extract())
    MyNewHeap.printHorizontalTree();